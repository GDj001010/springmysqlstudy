...
public class UploadServiceImpl implements UploadService {
	...
	public ResponseEntity<Resource> download(int attachNo, String userAgent) {
		...
		// 다운로드 할 첨부 파일의 정보 가져오기(경로, 원래 이름, 저장된 이름)
		AttachDTO attachDTO = uploadMapper.getAttachByNo(attachNo);
		
		// 다운로드 할 첨부 파일의 File 객체 -> Resource 객체로 변환
		File file = new File(attachDTO.getPath(), attachDTO.getFilesystemName());
		Resource resource = new FileSystemResource(file);	// Resource 객체로 변환
		...
		try {
			...
			if(userAgent.contains("Trident")) {	// contains < 포함여부 확인
				originName = URLEncoder.encode(originName, "UTF-8").replace("+", "");	// replace < +가 포함된다면 공백으로 바꾼다.
			}
	...
	public ResponseEntity<Resource> downloadAll(int uploadNo) {
		...
		// zip 파일을 생성하기 위한 Java IO Stream
		...
		try {
			...
			// 첨부 파일들을 하나씩 순회하면서 읽어 들인 뒤 zip 파일에 추가하기 + 각 첨부 파일들의 다운로드 
			for(AttachDTO attachDTO : attachList) {
				
				// zip 파일에 추가할 첨부 파일 이름 등록(첨부 파일의 원래 이름) 다운로드와 동일하다
				...
				// bin -> zout으로 파일 복사하기 (파일 복사 프로그램) Java 코드
				byte[] b = new byte[1024];	// 첨부파일을 1KB 단위로 읽겠다. ( 복사한다 )
				int readByte = 0;			// 1025크기일시 1024를 읽은 뒤 1이 남는다. 실제로 읽어들인 바이트 수이다.
				while((readByte = bin.read(b)) != -1) {
					zout.write(b, 0, readByte);	// 읽어들인 바이트 수까지만 읽으시오.
				}
				bin.close();
				zout.closeEntry();
	...
	public int updateUpload(MultipartHttpServletRequest multipartRequest) {
		...
		uploadDTO.setUploadContent(uploadContent);
		uploadDTO.setUploadTitle(uploadTitle);
		int updateResult = uploadMapper.updateUpload(uploadDTO);
